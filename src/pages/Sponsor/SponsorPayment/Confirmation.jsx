  ...
  useEffect(() => {
    ...
    const complete_order = async () => {
      if (!apiCalled) { // Check if API has already been called
        try {
          const response = await axios.post(`${apiurl}/api/razorpay/order/complete/`, {
          ...
          if (response && response.data.client_secret) { // Check for successful response
            ...
            sessionStorage.setItem("successMessage", "Event Sponsored successfully!");
    ...
    isSuccess ? (
      ...
      <>
        ...
        <p className="text-muted">
          {countdown > 0 ? `Redirecting in ${countdown} seconds...` : navigate("/sponsored_event")}
        </p>
      </>
    ) : (
      // Error Screen
