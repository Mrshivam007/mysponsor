...
import { ApproveWithdraw, ApproveContentWithdraw } from "../../../../../../redux/actions/paymentWithdraw";
...
export default function ManageTable(props) {
  ...
  const handlePaymentApprove = (id) => {
    dispatch(ApproveContentWithdraw(id));
  ...
  useEffect(() => {
    ...
    if (approveWithdraw) {
      if (approveWithdraw.msg == "content withdra approved") {
        console.log("Withdraw Approved Successfully");
        sessionStorage.setItem("deletionMessage", "Payment Withdraw has been Approved!!");
        navigate("/administrator/payment_content_withdraw");
        window.location.reload();
      } else {
